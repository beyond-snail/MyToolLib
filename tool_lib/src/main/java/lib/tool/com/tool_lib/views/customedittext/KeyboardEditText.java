package lib.tool.com.tool_lib.views.customedittext;
...
public class KeyboardEditText extends EditText implements KeyboardView.OnKeyboardActionListener {
    ...
    private boolean isAmount = false; //是否是金额键盘
    ...
    private onBackClickKey backClickKey;


    public interface onBackClickKey {
        void onBack();
    }


    public void setBackClickKey(onBackClickKey backClickKey) {
        this.backClickKey = backClickKey;
    }
    ...
    private void initKeyboard(Context context, AttributeSet attrs) {
        ...
        if (array.hasValue(R.styleable.Keyboard_xml)) {
            ...
            //获取自定义的属性值
            if (array.hasValue(R.styleable.Keyboard_isAmount)) {
                isAmount = array.getBoolean(R.styleable.Keyboard_isAmount, false);
                ;
            }
...
//                            mContentView.scrollBy(0, -temp);
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        ...
        } else if (0x0 <= primaryCode && primaryCode <= 0x7f && primaryCode != 0x2e) {
            //可以直接输入的字符(如0-9,.)，他们在键盘映射xml中的keycode值必须配置为该字符的ASCII码
            if (isAmount) {
                setTextAmount(Integer.valueOf(Character.toString((char) primaryCode)));
            } else {
    ...
    private void setBackAmount() {
        amount = amount / 10;
        if (amount == 0) {
            this.setText("");
            return;
        }
        this.setText(StringUtils.formatAmount(amount));
    ...
    private void showKeyboard() {
        if (null != mKeyboardWindow) {
            if (!mKeyboardWindow.isShowing()) {
                ...
                mKeyboardView.setKeyboard(mKeyboard);
                mKeyboardWindow.showAtLocation(this.mContentView, Gravity.BOTTOM, 0, 0);
...
//                        mContentView.scrollTo(0,mScrollDistance);
                    }
                }
