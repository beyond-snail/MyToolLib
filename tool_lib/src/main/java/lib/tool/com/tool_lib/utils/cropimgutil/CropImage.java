package lib.tool.com.tool_lib.utils.cropimgutil;
...
public class CropImage {
	...
	private Bitmap onSaveClicked(Bitmap bm) {
		...
		int width = 100;// dr.width(); // modify by yc
		int height = 100;// dr.height();
		Bitmap croppedImage = Bitmap.createBitmap(width, height,
				Bitmap.Config.RGB_565);
		{
			Canvas canvas = new Canvas(croppedImage);
	...
	public String saveToLocal(Bitmap bm) {
		String path = FILE_LOCAL + "_avatar.jpg";
		try {
	...
	class BackgroundJob implements Runnable {
		...
		public void run() {
			...
			try {
				...
				mHandler.sendMessage(mHandler.obtainMessage(REMOVE_PROGRESS));
				// mHandler.post(new Runnable()
				// {
				// public void run()
				// {
